...
def get_products(
    ...
    logger.info(f"[TOOL] get_products called: '{question}'")  # ADD
    try:
...
class AgentFrameworkProductManagementAgent:
    ...
    async def invoke(self, user_input: str, session_id: str) -> dict[str, Any]:
        ...
        logger.info(f"[AGENT] ProductManagerAgent invoked | session={session_id}")  # ADD
    ...
    def _get_agent_response(
        ...
        if structured_response and isinstance(structured_response, ResponseFormat):
            logger.info(f"[AGENT] Response status: {structured_response.status}")  # ADD
            response_map = {
                'input_required': {
                    'is_task_complete': False,
                    'require_user_input': True,
